...
import { ALL_CATEGORIES_LABEL, type GalleryItem } from '@/lib/gallery-data'
...
export default function GalleryContent({ items }: { items: GalleryItem[] }) {
  ...
  const filtered = activeCategory === ALL_CATEGORIES_LABEL
    ? items
    : items.filter((item) => item.category === activeCategory)

  // Nothing published for the homepage yet, or the API is down. Render nothing
  // rather than a broken section or stale placeholder images.
        ...
        <StaggerChildren className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {filtered.map((item, idx) => (
            <StaggerItem key={item.id}>
              <button
                onClick={() => setLightboxImage(items.findIndex((g) => g.id === item.id))}
      ...
      <AnimatePresence>
        {lightboxImage !== null && (
          ...
          >
            ...
            >
              <Image
                src={items[lightboxImage].image}
                alt={items[lightboxImage].altText}
                width={1200}
                height={800}
                className="max-h-[85vh] w-auto rounded-lg object-contain"
              />
              <p className="mt-3 text-center text-sm text-white/80">{items[lightboxImage].altText}</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
